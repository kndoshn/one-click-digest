# UI State Machine v1.4-UX

更新日: 2026-01-11

本ドキュメントは、content script 側の UI 状態（`UiState`）とイベント（`UiEvent`）の遷移を、実装に合わせて簡潔に定義します。
...
- `EXTRACTING`: 本文抽出中。
- `PREFLIGHT`: 見積もり表示中。`refining=true` の場合は token 再計測中。
...
### Confirm 画面の「3行で実行」
- UI イベントとしては `as:runLight`。
- controller が `BULLETS_3` を選択し直した上で、`TOKENS_REFINED` を使って estimate を更新し、`START_SUMMARY` に進める。
- state machine の追加分岐は増やさず、既存イベントで表現している。

### キャンセル/クローズ
- state machine 上のイベントというより controller 側で処理する。
  - background に `ABORT_RUN` を送る
  - `RESET`（banner 付き）または unmount
