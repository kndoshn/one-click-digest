...
async function refreshSettingsCache() {
    ...
    const modelFinal = res?.[STORAGE_KEYS.MODEL_FINAL] || DEFAULT_SETTINGS.modelFinal;
    const promptCachingEnabled = typeof res?.[STORAGE_KEYS.PROMPT_CACHING_ENABLED] === 'boolean'
        ? Boolean(res?.[STORAGE_KEYS.PROMPT_CACHING_ENABLED])
        : DEFAULT_SETTINGS.promptCachingEnabled;
    const promptCachingTtl = res?.[STORAGE_KEYS.PROMPT_CACHING_TTL] || DEFAULT_SETTINGS.promptCachingTtl;
    ...
    settingsCache = {
        ...
        maxArticleCharsToSend: Math.floor(parseNumberOr(res?.[STORAGE_KEYS.MAX_ARTICLE_CHARS_TO_SEND], DEFAULT_SETTINGS.maxArticleCharsToSend)),
        uiLanguage: res?.[STORAGE_KEYS.UI_LANGUAGE] || DEFAULT_SETTINGS.uiLanguage
    };
    settingsLoaded = true;
}
...
async function postJsonWithTimeout(args) {
    ...
    try {
        try {
            ...
            const message = (timedOut && 'Request timed out') ||
...
function normalizeApiError(res, fallback) {
...
function currentCacheControl() {
    if (!settingsCache.promptCachingEnabled)
        return undefined;
...
function headerLabelsFor(language) {
    const lang = normalizeLang(language);
    if (lang.startsWith('ja'))
        return { summary: '要約', conclusion: '結論' };
    if (lang.startsWith('fr'))
        return { summary: 'Résumé', conclusion: 'Conclusion' };
    if (lang.startsWith('de'))
        return { summary: 'Zusammenfassung', conclusion: 'Fazit' };
    if (lang.startsWith('es'))
        return { summary: 'Resumen', conclusion: 'Conclusion' };
    if (lang.startsWith('it'))
        return { summary: 'Sintesi', conclusion: 'Conclusione' };
    if (lang.startsWith('pt'))
        return { summary: 'Resumo', conclusion: 'Conclusão' };
    if (lang.startsWith('zh')) {
        if (lang.includes('tw') || lang.includes('hk') || lang.includes('mo'))
            return { summary: '摘要', conclusion: '結論' };
        return { summary: '摘要', conclusion: '结论' };
    }
    if (lang.startsWith('ko'))
        return { summary: '요약', conclusion: '결론' };
    return { summary: 'Summary', conclusion: 'Conclusion' };
}
function formatHintFor(mode, language) {
    if (mode === 'BULLETS_3')
        return 'Return exactly 3 bullet points, each starting with "- ".';
    if (mode === 'BULLETS_5')
        return 'Return exactly 5 bullet points, each starting with "- ".';
    if (mode === 'BULLETS_10')
        return 'Return exactly 10 bullet points, each starting with "- ".';
    const labels = headerLabelsFor(language);
    return ('Return EXACTLY 16 lines in this order:\n' +
...
function buildTokenCountPrompt(args) {
...
function buildSinglePrompt(args) {
...
function buildMapSystem(args) {
    return (`You are an assistant that extracts key points from article chunks.\n` +
        `Output language: ${args.language}.\n` +
...
function buildJoinedChunkSummaries(chunkSummaries) {
...
async function countTokens(args) {
    ...
    if (!res.ok)
        return normalizeApiError(res, 'Token count request failed.');
    ...
    (async () => {
        try {
            ...
            if (msg.type === 'COUNT_TOKENS') {
                ...
                const controller = new AbortController();
                if (runId)
                    abortRegistry.register(runId, controller);
                ...
                if (runId)
                    abortRegistry.unregister(runId, controller);
                if (!r.ok) {
