...
(function (AS) {
    ...
    (function (Chunker) {
        ...
        function roughHighTokens(chars, cjkChars) {
            if (chars <= 0)
                return 0;
        ...
        function splitLargeParagraph(para, targetTokens) {
            // Fallback: split by approximate char count; try to cut on whitespace/punctuation.
            const tokens = AS.Estimate.estimateTokens(para);
        ...
        function chunkText(text, targetTokens) {
            ...
            for (const p of paras0) {
                const t = AS.Estimate.estimateTokens(p).high;
        ...
        Chunker.chunkText = chunkText;
    })(Chunker = AS.Chunker || (AS.Chunker = {}));
})(AS || (AS = {}));
